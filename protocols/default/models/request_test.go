...
func TestNewRequestFromMessage(test *testing.T) {
	type args struct {
		message Message
	}
	...
	for _, data := range []struct {
	...
	}{
		{
			...
			want: Request{
				action: func() defaultProtocolModelValueTypes.Action {
					value, err := defaultProtocolModelValueTypes.NewAction([]byte("dummy"))
					require.NoError(test, err)
		...
		test.Run(data.name, func(test *testing.T) {
			got, err := NewRequestFromMessage(data.args.message)
...
func TestRequest_Action(test *testing.T) {
	type fields struct {
	...
	for _, data := range []struct {
		name   string
		fields fields
		want   defaultProtocolModelValueTypes.Action
	}{
		{
			name: "success",
			fields: fields{
			...
			want: func() defaultProtocolModelValueTypes.Action {
				value, err := defaultProtocolModelValueTypes.NewAction([]byte("dummy"))
				require.NoError(test, err)
...
func TestRequest_Headers(test *testing.T) {
	type fields struct {
...
func TestRequest_Body(test *testing.T) {
	type fields struct {
...
func TestRequest_ToMessage(test *testing.T) {
	type fields struct {
		action  defaultProtocolModelValueTypes.Action
		headers mo.Option[defaultProtocolModelValueTypes.Headers]
		body    mo.Option[defaultProtocolModelValueTypes.Body]
